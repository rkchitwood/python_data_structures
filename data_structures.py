...
def multiple_letter_count(phrase):
    ...
    frequencies = {}
    for ltr in phrase:
        if frequencies.get(ltr, False):
            frequencies[ltr] = frequencies[ltr] + 1
        else:
            frequencies[ltr] = 1
    return frequencies
...
def is_palindrome(phrase):
    ...
    return True if phrase.lower().replace(" ","") == reverse else False
...
def find_factors(num):
    ...
    [1, 293, 1097, 321421]
    """
    factors = [val for val in range(1, num // 2 + 1) if num % val == 0]
    factors.append(num)
    return factors
...
def two_list_dictionary(keys, values):
   ...
   """
    ...
    idx = 0
    for val in keys:
        double_dict[val] = values[idx] if idx < len(values) else None
        idx += 1
    return double_dict
...
def sum_up_diagonals(matrix):
    ...
    for idx in range(len(matrix)):
        sum += matrix[idx][idx]
        sum += matrix[idx][len(matrix)-idx-1]     
    return sum
